<?php
/*
Plugin Name: Improved Sale Badges - Free Version
...
*/
	...
	class WC_Improved_Sale_Badges_Free {
		...
		function isb_scripts() {
			wp_enqueue_style( 'isb-style', plugins_url( 'assets/css/style.css', __FILE__), '1.0.0' );
			wp_register_script( 'isb-scripts', plugins_url( 'assets/js/scripts.js', __FILE__), array( 'jquery' ), '1.0.0', true );
		...
		function isb_get_loop_badge() {
			...
			ob_start();
			$include = self::isb_get_path() . ( 'woocommerce/loop/sale-flash.php');
			include($include);
			$out = ob_get_clean();
		...
		function isb_get_single_badge() {
			...
			ob_start();
			$include = self::isb_get_path() . ( 'woocommerce/single/sale-flash.php');
			include($include);
			$out = ob_get_clean();
	...
	add_action( 'init', array( 'WC_Improved_Sale_Badges_Free', 'init' ), 998 );
